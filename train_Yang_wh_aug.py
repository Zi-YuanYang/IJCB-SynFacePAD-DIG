...
from dataset import TestDataset, ApplyWeightedRandomSampler, save_plts
from dataset import TrainDataset_withaug as TrainDataset
from utils import  AvgrageMeter, performances_cross_db, compute_video_score
from model.model import BaseModel, BaseMixModel, YangModel


def main(train_csv_1, train_csv_2, test_csv, log_file, intra_fe, args):
    ...
    if args.model_name != 'Yang':
        model = torch.nn.DataParallel(BaseModel(model_name=args.model_name,pretrained=False, num_classes=2))
    else:
        model = torch.nn.DataParallel(YangModel(model_name=args.model_name,pretrained=False, num_classes=2,intra_fea = intra_fe))
        print(model)
    model = model.cuda()
    ...
    for epoch in range(1, args.max_epoch+1):
        if os.path.isfile(os.path.join(checkpoint_save_dir, '{}.pth'.format(epoch))):
            ...
            lr_scheduler.step()
            log_file.write(f'Test: AUC_1= {AUC:.4f}, HTER_1= {HTER:.4f}\n')
            log_file.flush()
...
def train_epoch(epoch, model, train_loader_1, train_loader_2, optimizer, cen_criterion, current_lr, scaler, checkpoint_save_dir, log_file):
    ...
    for i in range(1, num_iter):
        ...
        input_data, label_1 = data_1["images"].cuda(), data_1["labels"].cuda()
...
def test_epoch(model, data_loader, video_format=False):
    model.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", default='Yang', type=str, help="model backbone")
    parser.add_argument("--csv_1", default='./SynthASpoof_data.csv', type=str, help="csv contains synth training data")
    parser.add_argument("--csv_2", default='./test.csv', type=str, help="csv contains real training data")
    parser.add_argument("--test_csv", default='./test.csv', type=str, help="csv contains testing data")

    parser.add_argument("--lr", default=0.001, type=float, help="initial learning rate")
    parser.add_argument("--input_shape", default=(224, 224), type=tuple, help="Neural Network input shape")
    parser.add_argument("--max_epoch", default=6, type=int, help="maximum epochs")
    parser.add_argument("--batch_size", default=14, type=int, help="train batch size")
    parser.add_argument("--intra_fe", default=1024, type=int, help="train batch size")
    ...
    # name = './logs/'+args.model_name
    logging_filename = os.path.join('logs/NoMixStyle',  '{}.txt'.format(args.prefix))
    if not os.path.isdir('logs/NoMixStyle'):
        os.makedirs('logs/NoMixStyle')
